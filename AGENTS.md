...
- **Frontmatter is YAML, and it fails silently.** An unquoted value containing `": "` (e.g. `description: ... Output: a clean PR`) or starting with `[` (e.g. `argument-hint: [a] [b]`) is a parse error, and Claude Code then loads the skill with **every frontmatter field dropped** — no name, no description, no `disable-model-invocation`. Use a folded block scalar (`description: >-`) for prose, and quote any `argument-hint` that starts with `[`. Validate with the marketplace-free-copy trick under "Testing the plugin" — the plain repo-root invocation will not catch it.
